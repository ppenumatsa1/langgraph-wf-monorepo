...
def test_config_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert cfg.runtime_target == "direct_langgraph"
...
def test_config_rejects_hosted_agent_runtime(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("RUNTIME_TARGET", "responses_wrapper")

    with pytest.raises(ValueError, match="Unsupported RUNTIME_TARGET"):
        get_config()
