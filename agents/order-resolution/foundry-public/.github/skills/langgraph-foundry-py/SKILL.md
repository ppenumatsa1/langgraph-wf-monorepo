...
description: Maintain this order-resolution service's LangGraph + Foundry hosted-agent runtime. Use for graph composition, langchain-azure-ai hosting, native Postgres checkpointers, interrupt/resume HITL flows, durable event projection, and workflow telemetry.
---
...
- Keep Foundry hosting Responses-native. Do not reintroduce legacy invocations adapters unless the task explicitly requires a new protocol and updates tests/contracts together.
...
1. `make test`
2. `make eval-backend`
3. `make eval-foundry` for hosted runtime, prompt, evaluator, or telemetry changes
