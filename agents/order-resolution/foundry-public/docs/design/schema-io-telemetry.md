...
These event names are stable even though the underlying runtime moves from MAF
to LangGraph.
...
- FastAPI health (`/health`, `/api/health`) and chat SSE request spans are
  excluded from request telemetry in the public lane so probes and long-lived
  streams do not obscure workflow signal.
