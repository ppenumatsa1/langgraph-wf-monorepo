                ...
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        ...
                        DO UPDATE SET
                            ...
                            updated_at = CASE
                                WHEN workflow_interrupts.status = 'resuming'
                                    THEN workflow_interrupts.updated_at
                                ELSE NOW()
                            END
                        """,
                        (
                ...
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        WHERE checkpoint_id = %s::uuid
                          ...
                          AND (decision IS NULL OR decision = %s)
                        RETURNING {_SELECT_COLUMNS}
                        """,
                        (decision, reviewer, comments, checkpoint_id, decision),
                    )
                    ...
                    if row["status"] not in {"resuming", "approved", "rejected"}:
        ...
        with self._pool.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    """,
                    (checkpoint_id, decision, claimed_at),
                )
