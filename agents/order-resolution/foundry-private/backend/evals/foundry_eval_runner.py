...
def _parse_evidence_timestamp(payload: dict[str, object], field: str) -> datetime:
    ...
    if not isinstance(value, str):
        raise ValueError(f"Private Foundry E2E evidence is missing {field}")
    try:
        parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise ValueError(f"Private Foundry E2E evidence {field} must be ISO 8601") from exc
    if parsed.tzinfo is None:
        raise ValueError(f"Private Foundry E2E evidence {field} must include a timezone")
    return parsed.astimezone(timezone.utc)


def _load_private_e2e_evidence(
    path: Path,
) -> tuple[datetime, datetime, list[str], str]:
    if not path.is_file():
        raise FileNotFoundError(f"Private Foundry E2E evidence is required: {path}")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("Private Foundry E2E evidence must be a JSON object")
    ...
    if generated_at < started_at:
        raise ValueError("Private Foundry E2E evidence generated_at cannot precede started_at")
    release_id = payload.get("release_id")
    if not isinstance(release_id, str) or not release_id:
        raise ValueError("Private Foundry E2E evidence is missing release_id")
    ...
    if len(conversation_ids) != 3:
        raise ValueError("Private Foundry E2E evidence must contain three conversation IDs")
...
def _secure_base_url(value: object, *, source: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise ValueError(f"{source} is missing a frontend URL")
    parsed = urlsplit(value.strip())
    if parsed.scheme != "https" or not parsed.hostname:
        raise ValueError(f"{source} frontend URL must use HTTPS")
    if parsed.username or parsed.password or parsed.query or parsed.fragment:
        raise ValueError(f"{source} frontend URL must not contain credentials or query data")
    return value.strip().rstrip("/")


def _frontend_url_from_evidence(path: Path) -> str | None:
    if not path.is_file():
        return None
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise ValueError(f"Deployment verification evidence must be an object: {path}")
    direct_url = payload.get("frontend_url")
    endpoints = payload.get("endpoints")
    nested_url = endpoints.get("frontend_url") if isinstance(endpoints, dict) else None
    value = direct_url or nested_url
    if value is None:
        return None
    return _secure_base_url(value, source="Deployment verification evidence")


def _azd_value(azd_dir: Path, key: str) -> str:
    command = [
        "azd",
        "env",
        "get-value",
    ...
    environment = os.getenv("FOUNDRY_AZD_ENV_NAME")
    if environment:
    ...
    if not value:
        raise ValueError(f"Selected Foundry azd environment is missing {key}")
    return value


def _private_frontend_url_from_azd(root: Path) -> str:
    azd_dir = Path(os.getenv("FOUNDRY_AZD_DIR", root.parent / "infra" / "foundry-hosted"))
    subscription_id = _azd_value(azd_dir, "AZURE_SUBSCRIPTION_ID")
    resource_group = _azd_value(azd_dir, "AZURE_RESOURCE_GROUP")
    frontend_name = _azd_value(azd_dir, "FRONTEND_CONTAINER_APP_NAME")
    completed = subprocess.run(  # noqa: S603
        [
            "az",
            "containerapp",
    ...
    return _secure_base_url(
        f"https://{fqdn}" if fqdn else "",
        source="Private Foundry deployment",
    )


def _workflow_base_url(root: Path, evidence_path: Path) -> str:
    configured_url = os.getenv("FOUNDRY_WORKFLOW_BASE_URL")
    if configured_url:
        return _secure_base_url(configured_url, source="FOUNDRY_WORKFLOW_BASE_URL")

    configured_evidence = os.getenv("FOUNDRY_DEPLOYMENT_VERIFICATION_FILE")
    candidates = (
        [Path(configured_evidence)]
        if configured_evidence
        else [
            evidence_path.with_name("deployment-verification.json"),
            evidence_path.with_name("verification.json"),
        ]
    )
    for candidate in candidates:
        frontend_url = _frontend_url_from_evidence(candidate)
        if frontend_url:
            return frontend_url
    return _private_frontend_url_from_azd(root)
...
def _load_workflow_messages(
    ...
    for conversation_id in conversation_ids:
        request = Request(
            f"{base_url}/api/workflows/{quote(conversation_id, safe='')}",
            headers={"Accept": "application/json"},
        )
        with urlopen(request, timeout=30) as response:  # noqa: S310
            workflow = json.loads(response.read().decode("utf-8"))
        if not isinstance(workflow, dict):
            raise ValueError("Selected-thread workflow snapshot must be a JSON object")
...
async def run_foundry_eval() -> None:
    ...
    eval_name = str(foundry_cfg.get("name", "order-resolution-foundry-private-report"))
    evaluation_name = f"{eval_name}-workflow"
    run_name = f"{eval_name} workflow run"
    poll_interval = float(
    ...
    started_at, generated_at, conversation_ids, release_id = _load_private_e2e_evidence(
        evidence_path
    )
    requested_release_id = os.getenv("FOUNDRY_RELEASE_ID")
    if requested_release_id and requested_release_id != release_id:
        raise ValueError(
            "Private Foundry E2E evidence does not belong to the active release window"
        )
    selected_conversation_ids = conversation_ids[:max_workflows]
    base_url = _workflow_base_url(root, evidence_path)
    ...
    report_path = Path(
        os.getenv(
            "FOUNDRY_EVAL_EVIDENCE_FILE",
            foundry_root / "results" / "foundry-report.json",
        )
    )
    report_path.parent.mkdir(parents=True, exist_ok=True)
    workflow_evaluation: dict[str, object] = {
        "provider": "foundry-private-workflow-snapshot",
        "source": "redacted-selected-thread-projection",
        "fresh_workflow_snapshots": True,
    ...
    payload: dict[str, object] = {
        ...
        # Retain the release-runner field until its evidence schema migrates.
        "trace_evaluation": workflow_evaluation,
    }
    ...
    try:
        ...
        if models_cfg is None:
            raise RuntimeError(
                "Foundry model configuration is missing. Set FOUNDRY_PROJECTS_ENDPOINT "
                "and FOUNDRY_MODEL_DEPLOYMENT_NAME."
            )
        ...
        eval_object = await openai_client.evals.create(
            ...
            testing_criteria=_build_workflow_testing_criteria(
                evaluators,
                judge_model,
            ),
        )
        eval_run = await openai_client.evals.runs.create(
            ...
            metadata={
                "deployment_lane": "foundry-private",
                "e2e_started_at": started_at.isoformat(),
                "conversation_count": str(len(selected_conversation_ids)),
                "snapshot_source": "redacted-selected-thread-projection",
            },
        ...
        payload.update(
            {
                ...
                "report_url": (
                    f"{models_cfg.project_endpoint.rstrip('/')}/evaluation/"
                    f"evaluations/{eval_object.id}/runs/{eval_run.id}"
                ),
    ...
    print(f"Foundry private workflow report saved to: {report_path}")
