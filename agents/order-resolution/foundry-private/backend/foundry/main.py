...
from app.core.database import is_connection_placeholder
...
_DEFAULT_HOSTED_AGENT_NAME = "order-resolution-private-hosted"
_HOSTED_CUSTOMER_ID = "foundry-private-hosted"
...
def _apply_runtime_database_url_override() -> None:
    ...
    if (
        not database_url
        or is_connection_placeholder(database_url)
        or database_host in {"localhost", "127.0.0.1"}
    ):
...
async def _run_from_responses(
    ...
    with tracer.start_as_current_span("foundry.responses.invoke") as span:
        ...
        _set_span_attribute(
            span,
            "gen_ai.agent.name",
            os.getenv("FOUNDRY_HOSTED_AGENT_NAME", _DEFAULT_HOSTED_AGENT_NAME),
        )
        _set_span_attribute(
            span,
            "gen_ai.agent.id",
            os.getenv("FOUNDRY_HOSTED_AGENT_ID", _DEFAULT_HOSTED_AGENT_NAME),
        )
        _set_span_attribute(span, "deployment.lane", "foundry-private")
                ...
                try:
                    await order_resolution_service.start_chat_run(
                        ChatRunRequest(
                            ...
                            customer_id=_HOSTED_CUSTOMER_ID,
                        )
                    )
...
def _initialize_app() -> Any:
    ...
    logger.info(
        "Foundry-private hosted LangGraph auto-tracing initialized: enabled=%s",
