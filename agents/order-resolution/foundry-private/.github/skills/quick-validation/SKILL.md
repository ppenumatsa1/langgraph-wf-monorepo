...
description: Run fast validation for low-risk private-lane app-only redeployments.
---

# Quick Validation Skill

Use this skill for routine private-lane redeployments where infrastructure,
private DNS, endpoint approvals, identity/RBAC, and API/HITL/event contracts
are unchanged.
...
`validate-quick` runs Playwright and, when `API_URL` is set, hosted smoke
checks. Hosted checks must use the approved external frontend URL; the
wrapper, Foundry, PostgreSQL, and ACR remain private.
...
- The private runner, private DNS, private endpoints, or managed-identity
  readiness changed or is not proven.

In those cases, run full validation.

Quick validation is not a deployment proof. It must fail closed on an
unexpected target or missing private-network prerequisite and must not fall
back to a public dependency endpoint.
