...
Use this skill before completing implementation work that changes backend,
frontend, workflow, evaluation, private-network infrastructure, or runnable
...
Run full validation (`make test`, `make eval-backend`, `make test-e2e`) when
workflow/HITL/contracts/IaC/private-network surfaces changed. Documentation
and skill-only work must still run the existing skills/frontmatter/link checks,
but it does not justify claiming a live deployment.
...
- A private runner, private DNS, endpoint approval, or Azure identity required
  for hosted checks is unavailable. Do not bypass that blocker by targeting a
...
- State whether validation is fully done or partially blocked. Local
  documentation/skill validation is distinct from private Azure readiness and
  never proves live Azure success.
