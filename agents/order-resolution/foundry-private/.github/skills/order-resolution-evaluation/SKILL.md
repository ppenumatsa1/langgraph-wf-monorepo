...
description: Run deterministic and private-hosted report-only evaluations for the Order Resolution workflow, using repository datasets/contracts locally and delegating remote Foundry evaluation mechanics to microsoft-foundry.
---

# Order Resolution Evaluation Skill

Use this skill for workflow, HITL, prompt, tool-routing, durable-event,
private-network, or hosted-runtime changes that need quality evidence.
...
2. Run private hosted report-only evaluation when runtime, prompts, or
telemetry-facing behavior changed:
...
- Run hosted evaluation only through the approved private runner and private
  Foundry path. Do not substitute a public endpoint or claim that a local
  result proves private Azure readiness.
- Application Insights is the required telemetry backend for hosted
  correlation. LangSmith is not required.
- Keep business approval cases intact; deployment previews and automation
  gates must not bypass workflow `interrupt()` / `Command(resume=...)`.
...
- Release-window-scoped Application Insights correlation for workflow,
  Foundry, dependency, and HITL spans when hosted evaluation is reported.
