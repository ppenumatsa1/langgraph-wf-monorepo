...
test.beforeEach(async ({ page }) => {
  await page.route("**/api/copilotkit/info", async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({ detail: "CopilotKit info is optional in tests" }),
    });
  });
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        events: [
          {
            ...
            timestamp: now,
            payload: {
              local_tool: "private-local-tool",
              mcp_tool: "private-mcp-tool",
              policy_retrieval: {
                provider: "private-rag-provider",
                query_id: "private-rag-query",
                count: 2,
              },
              evidence_count: 2,
              checkpoint_id: "private-checkpoint-id",
              result: {
                raw_model_output: "private-raw-model-output",
                query_id: "private-result-query",
              },
              secret_token: "private-secret-token",
            },
          },
          {
            id: "event-output",
            ...
            payload: {
              ...
              status: "resolved",
              raw_model_output: "private-output-model-text",
              checkpoint_id: "private-output-checkpoint",
            },
          },
        ],
        pending_approvals: [],
        latest_output: {
          message: "Resolution complete",
          status: "resolved",
          raw_model_output: "private-latest-model-output",
          connection_string: "private-postgres-connection-string",
          proposed_action: {
            action: "refund",
            order_id: "ORD-1001",
            amount: 10,
  ...
  await page.route(`**/api/chat/stream/${threadId}/rich`, async (route) => {
    await route.fulfill({
...
test("private frontend renders redacted workflow projections only", async ({ page }) => {
  await page.goto("/");

  await expect(page.locator(".panel-timeline")).toContainText("tool.call");
  await expect(page.locator(".panel-timeline")).toContainText("2 evidence item(s)");
  await expect(page.locator(".panel-output")).toContainText("Resolution complete");
  await expect(page.locator(".panel-output")).toContainText("Copy summary");
  await expect(page.locator(".panel-rag-evidence")).toContainText("Policy Evidence");
  await expect(page.locator("body")).not.toContainText("private-local-tool");
  await expect(page.locator("body")).not.toContainText("private-mcp-tool");
  await expect(page.locator("body")).not.toContainText("private-rag-provider");
  await expect(page.locator("body")).not.toContainText("private-rag-query");
  await expect(page.locator("body")).not.toContainText("private-checkpoint-id");
  await expect(page.locator("body")).not.toContainText("private-raw-model-output");
  await expect(page.locator("body")).not.toContainText("private-secret-token");
  await expect(page.locator("body")).not.toContainText(
    "private-postgres-connection-string",
  );
  await expect(page.getByText("Raw details")).toHaveCount(0);
  await expect(page.getByRole("button", { name: "Copy raw" })).toHaveCount(0);
});

test("API routes reject HTML fallback responses", async ({ page }) => {
  ...
  await expect(page.locator(".sidebar-history")).toContainText(
    "Expected application/json",
  );
  await expect(page.locator(".sidebar-history")).toContainText("received text/html");
  await expect(page.locator("body")).not.toContainText("SPA fallback");
});
...
test("optional AG-UI rejects HTML fallback streams", async ({ page }) => {
  await page.unroute(`**/api/chat/stream/${threadId}/ag-ui`);
  await page.route(`**/api/chat/stream/${threadId}/ag-ui`, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "text/html",
      body: "<!doctype html><title>AG-UI fallback</title>",
    });
  });
  ...
  await expect(page.getByRole("status")).toContainText(
    "The optional AG-UI endpoint returned a non-event-stream response.",
  );
  await expect(page.locator(".panel-timeline")).toContainText("workflow.output");
  await expect(page.locator("body")).not.toContainText("AG-UI fallback");
});
...
test("CopilotKit bridge rejects HTML fallback streams", async ({ page }) => {
  await page.route("**/api/copilotkit", async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "text/html",
      body: "<!doctype html><title>CopilotKit fallback</title>",
    });
  });
  ...
  await expect(page.getByRole("status")).toContainText(
    "The optional CopilotKit endpoint returned a non-event-stream response.",
  );
  await expect(page.locator("body")).not.toContainText("CopilotKit fallback");
});
...
test("CopilotKit bridge posts selected-thread AG-UI input to the root endpoint", async ({
  ...
  expect(requests[0]).toEqual({ threadId });
