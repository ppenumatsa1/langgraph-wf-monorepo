...
profile="$script_dir/profiles/foundry-private.env"
bootstrap_profile="$script_dir/profiles/foundry-private-bootstrap.env"
package_validation_profile="$script_dir/profiles/foundry-private-package-validation.env"
...
for candidate in "$profile" "$bootstrap_profile" "$package_validation_profile"; do
  ...
  for forbidden_key in \
    ...
    POSTGRES_OPERATOR_IP PRIVATE_RUNNER_SSH_PUBLIC_KEY \
...
grep -Fq 'foundry-private/infra/foundry-hosted|env set AZURE_SUBSCRIPTION_ID 7df95e88-701c-4693-af77-3159f83b558d' "$scratch_dir/azd.log"
grep -Fq 'env set AZURE_RESOURCE_GROUP rg-langgraph-ora-foundry-private' "$scratch_dir/azd.log"
grep -Fq 'env set AZURE_LOCATION eastus2' "$scratch_dir/azd.log"
grep -Fq 'env set NAME_PREFIX orderprivate' "$scratch_dir/azd.log"
